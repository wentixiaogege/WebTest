package edu.itu.localrest;

import javax.ws.rs.Consumes;
import javax.ws.rs.Path;
import javax.ws.rs.Produces;
import javax.ws.rs.core.MediaType;
import javax.ws.rs.core.Response;

import org.apache.log4j.Logger;

import com.cic.localserver.Command;
import com.cic.localserver.LocalServer;
import com.cic.localserver.RelayControlCommand;

import edu.itu.html.JSONResult;
import edu.itu.html.LocalServerLightManipulation;
import edu.itu.util.Log4jUtil;



@Path("/LightManipulationStillTest")
@Produces(MediaType.APPLICATION_JSON)
@Consumes(MediaType.APPLICATION_JSON)
public class LocalLightManipulation {
	Logger logger = Log4jUtil.getLogger(LocalLightManipulation.class);
	public static LocalServer localServer;
	
	
	public Response doLightManipulation(LocalServerLightManipulation t){
		
		localServer = LocalServer.getLocalServer();
		
		
		// need to wait to see the results immediatly???????
		
		
		
	
		
		
		
		
		JSONResult result = new JSONResult(Response.Status.INTERNAL_SERVER_ERROR.getStatusCode(), "failed");
		try {
			logger.debug("testhere");
			
			
//			Command command = new RelayControlCommand("Relay Control",0,0,0,0);//(byte)commandID, dataLength, controlRelay, smartMeterTable[smartMeterID]
			
			// localserver
//			localServer.commandManager.setGeneratedCommand(localServer.coordinatorArray[coordinatorID], command);

			
			
			result.setHttpcode(Response.Status.OK.getStatusCode());
			result.setResult("success");
		} catch (Exception e) {
			e.printStackTrace();
			// better for debugging
			return Response.status(Response.Status.INTERNAL_SERVER_ERROR.getStatusCode()).entity(e.getMessage()+e.getStackTrace()).build();
		}
		
		return Response.status(200).entity(result).build();
	}
}
